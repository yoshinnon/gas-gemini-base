...
### Step 6: Apps Script API を有効化する

**clasp push を初めて実行する前に必須の設定です。** 一度だけ行えば以降は不要です。

1. ブラウザで以下の URL を開く
   ```
   https://script.google.com/home/usersettings
   ```
2. **「Google Apps Script API」** のトグルを **オン** に切り替える
3. 1〜2分待つ（設定の反映に時間がかかる場合があります）

> ⚠️ この手順をスキップすると `npm run push` 時に以下のエラーが出ます:
> `User has not enabled the Apps Script API.`

---

### Step 7: ビルドして GAS へプッシュ
...
### Step 8: Gemini API キーをスクリプトプロパティに設定
...
### `Push failed. User has not enabled the Apps Script API` エラー

Step 6 の手順で Apps Script API を有効化してください。有効化直後は反映まで 1〜2 分かかります。
